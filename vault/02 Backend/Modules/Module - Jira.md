...
- `createIssue(platform, issue)` → `POST /rest/api/3/issue`; project = `platform.jiraProjectKey`, summary = `[<referenceNo>] <description first 200 chars>`, issuetype `Task`, priority mapped via `mapPriority`. Returns the Jira key (e.g. `SUP-123`); throws on non-ok or missing key.
...
- `toAdf(text)`: wraps plain text in Atlassian Document Format (required for v3 rich-text fields).
...
- **Idempotent create:** the `jiraIssueKey`/`PENDING`/`SYNCED` guard prevents duplicate Jira issues from re-delivered events.
