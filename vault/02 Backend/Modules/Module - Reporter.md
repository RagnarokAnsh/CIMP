...
- **`createIssue(ctx, dto, files)`** — sniffs → upserts reporter → writes buffers to `StorageService.save` **outside** the transaction (returns `storageKey`) → `createIssueWithUniqueReference` (best-effort `StorageService.delete` on every stored blob if that ultimately throws, so a failed persist doesn't orphan files) → emits `IssueEvents.CREATED` → returns `getIssueForReporter`.
...
- **Storage write is outside the DB transaction**; the reference-collision retry can re-run the insert reusing the same `storageKey`s, and a terminal persist failure now cleans the blobs up via `StorageService.delete` (idempotent).
- `sanitizeContext` (the SDK-diagnostics clamp) skips `__proto__`/`constructor`/`prototype` keys — reporter-controlled JSON can't hand downstream code an object with a hijacked prototype (defensive; not a live RCE).
