...
- `verifyToken(token)` → verifies HS256, then delegates to `AuthService.upsertFromClaims`. **Returns `null` on any error** (caught) so the guard cleanly 401s.
...
- If found + drifted `name`/`email`, updates and saves. If not found, creates a new `StaffUser` (signature already trusted).
...
- **SSE ticket vs session token are audience-partitioned** — a session JWT (no `aud`) is rejected on the SSE route, and the 30s `aud:'sse'` ticket is rejected on normal routes.
